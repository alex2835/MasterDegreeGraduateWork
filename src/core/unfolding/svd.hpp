...
//	// ctor
//	svd();
//	void Factorization( std::vector<std::vector<Float>> matrix, std::vector<std::vector<Float>>& s,
//						std::vector<std::vector<Float>>& u, std::vector<std::vector<Float>>& v );
//	void VerifyFactorisition( std::vector<std::vector<Float>>& u, std::vector<std::vector<Float>>& s,
//							  std::vector<std::vector<Float>>& v );
//	int GetRank( std::vector<std::vector<Float>>& s );
//	void GetPseudoinverse( std::vector<std::vector<Float>>& s,
//						   std::vector<std::vector<Float>>& u, std::vector<std::vector<Float>>& v,
//						   std::vector<std::vector<Float>>& pseudoinv );
//	void VerifyPseudoinverse( std::vector<std::vector<Float>>& A,
//							  std::vector<std::vector<Float>>& A_pesudoinv );
//
//private:
//	// M_target is (A^t * A - eigenvalue*I)
//	void print_m( std::vector<std::vector<Float>> matr, std::string operation );
//	void prod_matrix( std::vector<std::vector<Float>> A,
//					  std::vector<std::vector<Float>>& B, std::vector<std::vector<Float>>& Prod );
//	void transpose_matr( std::vector<std::vector<Float>> A,
//						 std::vector<std::vector<Float>>& A_t );
//	void GaussJordanElimination( std::vector<std::vector<Float>> matrix, std::vector<Float>& eigen_vec );
//	void ConjugateFor_M_target( std::vector<Float> eigen_vec, std::vector<std::vector<Float>>& h_matrix );
//	void InverseConjugateFor_M_target( std::vector<Float> eigen_vec, std::vector<std::vector<Float>>& ih_matrix );
//	void InverseDiagMatrix( std::vector<std::vector<Float>> matrix,
//							std::vector<std::vector<Float>>& inv_matrix );
//	void ReduceMatrix( std::vector<std::vector<Float>> matrix,
//					   std::vector<std::vector<Float>>& r_matrix, std::size_t new_size );
//	void GetEigenvalsEigenvecs( std::vector<std::vector<Float>> matrix,
//								std::vector<Float>& eigenvalues, std::vector<std::vector<Float>>& eigen_vecs, std::size_t eig_count );
//	void print_eigenv( std::vector<Float> eigenv, std::string operation );
//
//private:
//	bool track;
...
//
//// S U V
//std::tuple<Mat, Mat, Mat> SVD( const Mat& mat )
//{
//	Mat s, u, v;
//	svd alg;
//	alg.Factorization( mat.GetRaw(), s.GetRaw(), u.GetRaw(), v.GetRaw() );
//	return { s, u, v };
//}
//
//
...
//}
//
//void svd::Factorization(std::vector<std::vector<Float>> A, std::vector<std::vector<Float>>& s,
//		std::vector<std::vector<Float>>& u, std::vector<std::vector<Float>>& v)
//	{
//
//		std::vector<std::vector<Float>> A_t, A_t_x_A, u_1, v_1, s_inverse, av_matrix;
...
//void svd::InverseDiagMatrix(std::vector<std::vector<Float>> A,
//		std::vector<std::vector<Float>>& inv_A)
//	{
...
//
//		for (std::size_t index = 0; index < m_size; index++) {
//			if (A[index][index] != (Float)0.0 ) {
//				inv_A[index][index] = (Float)1.0 /A[index][index];
//			}
//			else {
//				inv_A[index][index] = (Float)0.0;
//			}
//		}
//	}
//
//
//void svd::prod_matrix(std::vector<std::vector<Float>> A,
//		std::vector<std::vector<Float>>& B, std::vector<std::vector<Float>>& prod){
//		prod.resize(A.size());
//		for (std::size_t row = 0; row < A.size(); row++){
//			prod[row].resize(A[row].size());
//			std::fill(prod[row].begin(), prod[row].end(), Float(0.0));
//		}
//
...
//}
//
//void svd::transpose_matr(std::vector<std::vector<Float>> A,
//		std::vector<std::vector<Float>>& A_t){
//
//	std::size_t m_size = A.size();
//
...
//}
//
//Float RandomNumber() { return Float(std::rand() % 10000); }
//
//// Recursive computation of eigenvalues and eigen-vectors
//// Compute eigenvalue[i] and eigen_vecs[i], where i = eig_count
//void svd::GetEigenvalsEigenvecs(std::vector<std::vector<Float>> matrix,
//	std::vector<Float>& eigenvalues, 
//	std::vector<std::vector<Float>>& eigen_vecs, 
...
//		}
//
//		std::vector<Float> m; m.resize(m_size);
//		std::vector<Float> m_temp; m_temp.resize(m_size);
//
//		Float lambda_old = 0;
//
...
//				Float lambda = (index > 0) ? (m[0] / m_temp[0]) : m[0];
...
//
//			std::vector<std::vector<Float>> H_matrix_prod;
//			prod_matrix(H, matrix, H_matrix_prod);
//
//			// inverse matrix for H: inv_H
//			std::vector<std::vector<Float>> inv_H;
//			InverseConjugateFor_M_target(eigen_vec, inv_H);
//
//			std::vector<std::vector<Float>> inv_H_matrix_prod;
//
...
//		if (m_size <= 1)
//		{
//			for (std::size_t idx = 0; idx < eigenvalues.size(); idx++)
//			{
...
//				GaussJordanElimination(M_target, eigen_vecs[idx]);
//
//				// Normalize eigen vectors
//				Float eigsum_sq = 0;
//
//				for (std::size_t v = 0; v < eigen_vecs[idx].size(); v++)
//					eigsum_sq += std::pow(eigen_vecs[idx][v], (Float)2.0);
//
//				for (std::size_t v = 0; v < eigen_vecs[idx].size(); v++)
//					eigen_vecs[idx][v] /= std::sqrt(eigsum_sq);
//
//				// Essentially eigenvalues[idx] should be positive for symmetric matrix,
//				// However, negative values or very small values 
//				// may occur due to the accuracy of the calculations.
//				if (eigenvalues[idx] < 10e-5)
//					eigenvalues[idx] = 0;
//
//				eigenvalues[idx] = std::sqrt(eigenvalues[idx]);
//			}
//
//			return;
//		}
...
//void svd::ReduceMatrix(std::vector<std::vector<Float>> matrix,
//		std::vector<std::vector<Float>>& reduced_matr, std::size_t new_size){
//		if (new_size > 1)
//		{
...
//}
//
//void svd::ConjugateFor_M_target(std::vector<Float> eigen_vec,
...
//
//		h_matrix[0][0] = (Float)1.0 / eigen_vec[0];
//
...
//}
//
//void svd::InverseConjugateFor_M_target(std::vector<Float> eigen_vec, std::vector<std::vector<Float>>& ih_matrix){
...
//void svd::GaussJordanElimination(std::vector<std::vector<Float>> M, std::vector<Float>& eigen_vec) {
//
//		for (std::size_t s = 0; s < M.size() - 1; s++)
//		{
//			const Float diag_elem = M[s][s];
...
//}
//
//void svd::VerifyFactorisition(std::vector<std::vector<Float>>& u, std::vector<std::vector<Float>>& s,
//	std::vector<std::vector<Float>>& v) {
//
//	std::vector<std::vector<Float>> prod_US, prod_USVt, v_transp;
//
...
//}
//
//void svd::VerifyPseudoinverse(std::vector<std::vector<Float>>& A, std::vector<std::vector<Float>>& A_pesudoinv) {
//
...
//
//
//int svd::GetRank(std::vector<std::vector<Float>>& s) {
//	int rank = 0;
//
...
//}
//
//void svd::GetPseudoinverse(std::vector<std::vector<Float>>&  s, 
//	std::vector<std::vector<Float>>&  u, std::vector<std::vector<Float>>&  v,
//	std::vector<std::vector<Float>>&  pseudoinv){
//
//	std::vector<std::vector<Float>> s_inv, prod_VS_inv, u_transp;
...
//}
//
//void svd::print_m(std::vector<std::vector<Float>>	M, std::string operation){
//
...
//}
//
//void svd::print_eigenv(std::vector<Float> eigenv, std::string operation) {
//
